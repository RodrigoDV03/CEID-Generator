...
from core.fases.services import PaymentService, DescriptionService, DocumentGeneratorService
...
class TdrBuilder:
    ...
    def generar(
        ...
        # Obtener plantilla
        if self.config.es_administrativo:
            ruta_template = PathUtils.ruta_absoluta_relativa('./data/modelos/tdr_administrativo.docx')
        else:
            # Para docentes, usar plantilla según categoría
            ruta_template = PathUtils.ruta_absoluta_relativa(
                f'./data/modelos/tdr_tipo{docente.categoria_letra}_.docx'
            )
