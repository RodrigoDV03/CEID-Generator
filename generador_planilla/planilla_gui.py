...
def iniciar_interfaz_planilla(callback_volver=None):
    ...
    # ---------------- SECCIÓN: Mes y carga ----------------
    etiqueta(card, "Selecciona el mes:")

    crear_option_menu(card, variable=mes_var, opciones=meses)

    lbl_carga = ctk.CTkLabel(card, text="Número de carga:", font=FONT_SECTION, text_color=WHITE_COLOR)
    lbl_carga.pack(anchor="w", padx=20, pady=(10, 5))
    ctk.CTkRadioButton(card, text="1 (Primera carga)", variable=carga_var, value=1, text_color=WHITE_COLOR).pack(anchor="w", padx=35)
    ctk.CTkRadioButton(card, text="2 (Segunda carga)", variable=carga_var, value=2, text_color=WHITE_COLOR).pack(anchor="w", padx=35, pady=(0, 10))
    ...
    def crear_selector_archivo(master, texto_label, extensiones, actualizar_func):
        ...
        label_estado = ctk.CTkLabel(container, text="📂 No seleccionado", text_color=GRAY_COLOR)
    ...
    crear_selector_archivo(card, "Archivo de cursos", ("Archivos CSV", "*.csv"), actualizar_cursos)
    crear_selector_archivo(card, "Lista de docentes", ("Archivos Excel", "*.xlsx *.xls"), actualizar_docentes)
    crear_selector_archivo(card, "Docentes para Examen de Clasificación", ("Archivos Excel", "*.xlsx *.xls"), actualizar_clasif)
